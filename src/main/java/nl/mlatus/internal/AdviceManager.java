...
public class AdviceManager {
    ...
    public static void init(Map<String, String> option, Instrumentation inst){
        ...
        if(moduleDir.exists() && moduleDir.isDirectory()){
            ...
            HookLogger.warning("Loading modules failed: " + moduleDirPath + " does not exist or is not a directory.");
        }
    }
    ...
    private static void loadModulesFromJar(File file) {
        ...
        HookLogger.info("Loading modules from: " + jarPath + "...");
...
//                URLClassLoader cl = new URLClassLoader(urls, MethodHookParam.class.getClassLoader());
                ...
                for(String moduleClass : moduleClasses){
                    HookLogger.info("Loading module " + moduleClass + " from " + jarPath + "...");

                    try {
                        Class module = cl.loadClass(moduleClass);
                        if(!ClassHook.class.isAssignableFrom(module)){
                            HookLogger.warning("Module " + moduleClass + " does not extends ClassHook, skip it.");
                            continue;
                        }
                        ...
                        try {
                            ...
                            HookLogger.warning("Failed to create new instance for " + moduleClass + ", module not loaded.");
                        }

                    } catch (ClassNotFoundException e) {
                        HookLogger.warning("Failed to load module " + moduleClass + ": Class not found. skip it.");
                    }
                }

            } else {
                HookLogger.warning(file.getAbsolutePath() + " does not have the entry \"Advice-Module\" in the MANIFEST.MF, skip it.");
            }
        } catch (IOException e) {
            HookLogger.warning("Loading modules from " + file.getAbsolutePath() + " failed, skip it: " + e.getCause());
        }
    }
}
